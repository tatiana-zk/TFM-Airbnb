# -*- coding: utf-8 -*-
"""
@author: Guille (profe) & grupo 1 UCM clase 2 (2025-2026)
Integrantes Grupo 1 TFM Airbnb
...
def tr_tst_eval_lin(formula,data,seed=12345):
    ...
    X_tr, X_tst, y_tr, y_tst = train_test_split(X, y, test_size=0.2, random_state=seed)
    ...
    print("Root Mean squared error TEST: %.2f" % np.sqrt(mean_squared_error(y_tst, y_pred)))
...
## Funciones para modelo AIRBNB

##  Agrupar noches

def agrupar_noches(n):
    if n <= 3:
        return '1-3 noches'
    elif n <= 7:
        return '4-7 noches'
...
def clasificar_licencia(val):
    if pd.isna(val):
        return 'Sin dato'
    v = val.upper()
    tiene_numero = bool(re.search(r'(HUTB|HB-|AJ\d|ES[A-Z]{2,4}\d{5,})', v))
    tiene_exento = 'EXEMPT' in v
    if tiene_numero:
...
#Función de agrupación raras
def agrupar_raras(serie, umbral_pct=1.0):
    frecuencias = serie.value_counts(normalize=True) * 100
    categorias_frecuentes = frecuencias[frecuencias >= umbral_pct].index
    return serie.apply(lambda x: x if x in categorias_frecuentes else 'Otros')


# deteccion de outliers:

def resumen_outliers(serie, factor=1.5):
    q1, q3 = serie.quantile([0.25, 0.75])
    iqr = q3 - q1
    lim_inf = q1 - factor * iqr
    lim_sup= q3 + factor * iqr
    outliers = serie[(serie < lim_inf) | (serie > lim_sup)]
    return{'iim_inf':lim_inf, 'lim_sup':lim_sup,
           'min':serie.min(), 'max': serie.max(),
           'n_outliers': outliers.shape[0],
           'pct_outliers': round(outliers.shape[0] / serie.shape[0] * 100, 2)}

#funcion de cramer

def cramers_v(x, y, bins=5):
    x = pd.Series(x).reset_index(drop=True)
    y = pd.Series(y).reset_index(drop=True)
    if pd.api.types.is_numeric_dtype(x) and x.nunique() > bins:
        x = pd.qcut(x, q=bins, duplicates='drop')
    if pd.api.types.is_numeric_dtype(y) and y.nunique() > bins:
        y = pd.qcut(y, q=bins, duplicates='drop')
...
# Función para facilitar el cálculo del vif
from statsmodels.stats.outliers_influence import variance_inflation_factor

def vif_modelo(formula,data):

    # Matriz explícita de diseño y var objetivo por separado (extensión a dummies incluida)
    y, X = patsy.dmatrices(formula, data, return_type="dataframe")

    # Calcula VIF para las variables
    vif_data = pd.DataFrame()
    vif_data["feature"] = X.columns
    vif_data["VIF"] = [variance_inflation_factor(X.values, i) for i in range(X.shape[1])]

    # Elimina la pendiente y ordena
    vif_tabla = vif_data[vif_data['feature'] != 'Intercept'].sort_values(by='VIF', ascending=False)
